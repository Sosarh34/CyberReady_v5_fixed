...
def seed_phish_questions(conn):
    phish = [
        ("Email", "FROM: payroll@payr0ll-update.com\n---\nSUBJECT: URGENT — Salary Account Verification\n---\nYour salary payment is on hold. Verify your bank details within 30 minutes or your payment may be delayed.\n---\n[ VERIFY NOW ]", "The sender uses a lookalike domain", "The message is about salary", "The message asks for bank details under time pressure", "The email is short", "C", "The strongest clue is the urgent request for bank details. Verify salary requests through a known HR channel instead of replying or clicking.", "Easy"),
        ("Email", "FROM: it-support@micr0soft-security.com\n---\nSUBJECT: Microsoft 365 password expires today\n---\nYour mailbox will be disabled unless you confirm your password immediately.\n---\n[ OPEN MICROSOFT 365 ]", "The sender domain contains a lookalike spelling (micr0soft uses zero)", "The message mentions Microsoft 365", "The message asks you to act quickly", "The button is blue", "A", "The sender uses micr0soft-security.com, where the 0 replaces the letter o. It is not an official Microsoft domain. Use your trusted Microsoft 365 bookmark instead.", "Easy"),
...
def repair_phish_questions(conn):
    repairs = [
        ("FROM: payroll@%", "FROM: payroll@payr0ll-update.com\n---\nSUBJECT: URGENT — Salary Account Verification\n---\nYour salary payment is on hold. Verify your bank details within 30 minutes or your payment may be delayed.\n---\n[ VERIFY NOW ]", ("The sender uses a lookalike domain", "The message is about salary", "The message asks for bank details under time pressure", "The email is short"), "The strongest clue is the urgent request for bank details. Verify salary requests through a known HR channel.", "C"),
        ("FROM: it-support@%", "FROM: it-support@micr0soft-security.com\n---\nSUBJECT: Microsoft 365 password expires today\n---\nYour mailbox will be disabled unless you confirm your password immediately.\n---\n[ OPEN MICROSOFT 365 ]", ("The sender domain contains a lookalike spelling (micr0soft uses zero)", "The message mentions Microsoft 365", "The message asks you to act quickly", "The button is blue"), "The sender uses micr0soft-security.com, where the 0 replaces the letter o. It is not an official Microsoft domain.", "A"),
        ("FROM: Unknown number%", "FROM: Unknown number (+234 810 555 0199)\n---\nWHATSAPP: CEO needs a quick favour\n---\nI’m in a meeting and need you to buy ₦200,000 worth of gift cards. Send the codes here when done. Please don’t call because I’m busy.\n---\nREQUEST: Send gift-card codes", ("The sender is an unknown number impersonating the CEO", "The request is urgent and asks for gift-card codes", "The person uses WhatsApp", "The sender says they are busy"), "An unknown number claiming to be the CEO is impersonating a trusted person. Verify through a known company channel before acting.", "A"),
        ("FROM: recruitment@%", "FROM: recruitment@company-careers.com\n---\nSUBJECT: Final interview document\n---\nPlease open the attached file to view your interview schedule. We need it opened today.\n---\nDOWNLOAD: interview-schedule.example/Final_Interview.exe", ("It concerns recruitment", "The subject sounds professional", "The message asks you to open an executable file", "The download link leads to an executable file"), "The download destination ends in .exe and is presented through an unsolicited message. Do not open it; verify through the official recruitment channel.", "D"),
        ("FROM: +234 800%", "FROM: +234 800 000 0000\n---\nSMS: Your bank account will be blocked today\n---\nConfirm your BVN and PIN immediately to prevent account closure.\n---\nLINK: hxxps://secure-bank-verification.example", ("It says your account will be blocked", "It asks for a PIN and uses an unsolicited link", "It uses the word secure", "It is an SMS"), "The unsolicited link asks for a PIN and sensitive information. Use the official banking app or known contact channel instead.", "D"),
        ("FROM: benefits@%", "FROM: benefits@company.com\n---\nSUBJECT: Staff bonus — action required\n---\nWe have credited your bonus. To release it, sign in using the link below.\n---\nVisible: company.com  |  Actual: login-company-bonus.example", ("The sender looks like a company address", "The subject mentions a bonus", "The message asks you to sign in", "The visible and actual link destinations do not match"), "A mismatch between a visible link and its real destination is a strong phishing clue. Do not rely on link text alone.", "D"),
        ("FROM: ceo.office@%", "FROM: ceo.office@company.com\n---\nSUBJECT: Need a quick favour\n---\nSend me the employee list with phone numbers and home addresses. I need it in 10 minutes.\n---\nREQUEST: Send employee list", ("The sender uses a company-looking address", "The subject is short", "The request asks for sensitive employee information", "The message is one paragraph"), "The message requests sensitive personal information with urgency. Verify the request independently before disclosure.", "C"),
        ("FROM: HR Updates%", "FROM: HR Updates\n---\nWHATSAPP: Staff welfare payment\n---\nCongratulations! You have been selected for a staff welfare payment. Send your BVN, NIN and bank login details to this number to receive ₦150,000 today.\n---\nREQUEST: Send BVN, NIN and bank login details", ("It mentions staff welfare", "It contains a money amount", "The message asks for BVN, NIN and bank login details", "It comes through WhatsApp"), "The message body requests highly sensitive identity and banking information. Never send these details through an unsolicited WhatsApp message.", "C"),
        ("FROM: support@%", "FROM: support@company-secure.example\n---\nSUBJECT: Account verification required\n---\nWe detected unusual activity. Confirm your password to keep your account active.\n---\nREQUEST: Confirm password", ("The sender domain is unfamiliar", "The subject asks for verification", "The message asks you to submit your password", "The message mentions unusual activity"), "An unsolicited message asking for a password is a strong phishing signal. Never disclose your password.", "C"),
        ("FROM: Facilities Desk%", "FROM: Facilities Desk\n---\nSUBJECT: Visitor access update\n---\nA contractor is waiting at reception. Please let them into the restricted office because their access card is not working.\n---\nREQUEST: ALLOW ACCESS", ("The sender is called Facilities Desk", "The subject mentions visitor access", "The message asks you to make an exception to physical access controls", "The button says Allow Access"), "Requests to bypass physical access controls should follow the organisation's visitor procedure, even when the request appears legitimate.", "C"),
    ]
    for pattern, message, options, explanation, answer in repairs:
        conn.execute("""
            UPDATE questions
            SET question=?, option_a=?, option_b=?, option_c=?, option_d=?, explanation=?, correct_answer=?
            WHERE mode='phish' AND question LIKE ?
        """, (message, *options, explanation, answer, pattern))
